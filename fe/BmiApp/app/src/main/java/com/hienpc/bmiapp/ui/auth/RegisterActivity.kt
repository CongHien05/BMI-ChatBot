...
class RegisterActivity : AppCompatActivity() {
    ...
    private fun observeViewModel() {
        ...
        viewModel.authSuccess.observe(this, Observer { authData ->
            authData?.let { (token, email) ->
                // Đăng ký thành công
                // Hiển thị Toast chào mừng với tên (email)
                val displayName = email.substringBefore("@") // Lấy phần trước @ làm tên
                Toast.makeText(
                    this,
                ...
                lifecycleScope.launch {
                    authPreferences.saveToken(token, email)
                }
