...
class AuthViewModel : ViewModel() {
    ...
    // LiveData cho success state - chứa Pair<token, email> khi đăng nhập/đăng ký thành công
    private val _authSuccess = MutableLiveData<Pair<String, String>?>() // Pair<token, email>
    val authSuccess: LiveData<Pair<String, String>?> = _authSuccess
     ...
     * @param fullName: Họ tên hiển thị (optional)
     */
            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    if (!token.isNullOrBlank()) {
                        val responseEmail = body?.email ?: email
                        // Thành công - trả về token và email
                        _authSuccess.value = Pair(token, responseEmail)
                    } else {
